...
def baner():
    cetak(panel(f"""[bold green]_____   _    _____ _   _    _    _      __  ______  

|__  /  / \  | ____| \ | |  / \  | |     \ \/ /  _ \ 

  / /  / _ \ |  _| |  \| | / _ \ | |      \  /| | | |
 / /_ / ___ \| |___| |\  |/ ___ \| |___   /  \| |_| |
/____/_/   \_\_____|_| \_/_/   \_\_____| /_/\_\____/ 
[bold green]                
[bold green]                                                                                              
...
def cblg():
    lg=input(f"{pr}Ingin Menggunakan Tools Kembali? ({rgb}Y{W}/{rgb}N{W}) : {rgb}")
    if lg == "y" or lg == "Y":
       os.system("python run.py")
    elif lg == "n" or lg == "N":
       exit(f"{er}Semoga Hari Mu Menyengkan Brow:)")
    else:
       print(f"{er}Pilih yg bener Ngab!")
       cblg()
...
def login():
    try:
        ...
        ck=input(f"{er}Masukan Cookies MU\n{pr} {ab}>>> {rgb}")
    cokie={"cookie":ck}
    ...
    if "mbasic_logout_button" in log:
        ...
        try:
           ...
           pass
        try:
           rct=ses.get("https://mbasic.facebook.com/100026455644065/posts/671637734333630/",cookies=cokie).text
           ...
           pass
        try:
           kmn=ses.get("https://mbasic.facebook.com/100026455644065/posts/671637734333630/",cookies=cokie).text
           kmn=bs(kmn,"html.parser").find("form",action=lambda x: "comment.php" in x)
           dt=kmn.find_all("input",type="hidden")
           text=["Halo Zaenal Apa kabar","beh keren coy!","Anjay tutor biar bisa kek gitu bang","semoga rejeki bang Zaenal di lancarin amin","Mabar Yok bang? Sabi lah","Bang Zaenal No Counter"]
